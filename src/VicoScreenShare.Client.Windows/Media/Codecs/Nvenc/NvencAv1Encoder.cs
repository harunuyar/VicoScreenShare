...
/// <summary>
/// Direct NVENC SDK AV1 encoder. Mirrors <see cref="NvencH264Encoder"/>
/// almost verbatim — the lifecycle (session open → preset config →
/// initialize → owned input pool → async event pump → encode → bitstream
/// lock) is codec-agnostic. AV1-specific differences:
/// <list type="bullet">
///   <item><description><c>encodeGUID = NV_ENC_CODEC_AV1_GUID</c></description></item>
///   <item><description><c>presetGUID</c> from <see cref="NvencGuids.PresetByLevel"/>
///     same as H.264 — preset GUIDs are codec-agnostic.</description></item>
///   <item><description>Profile = <see cref="NvencGuids.Av1ProfileMain"/></description></item>
///   <item><description>Codec config overlay is <see cref="NV_ENC_CONFIG_AV1_OVERLAY"/>
///     instead of <see cref="NV_ENC_CONFIG_H264_OVERLAY"/>; bit positions
///     differ — see <see cref="Av1ConfigBits"/>.</description></item>
///   <item><description>Output is an AV1 OBU stream rather than H.264
///     Annex-B NAL units. The <c>repeatSeqHdr</c> bit flushes a Sequence
///     OBU with every Key Frame so receivers can decode mid-stream.</description></item>
/// </list>
///
/// Available only on RTX 40+ / Intel Arc / AMD RDNA 3+. The
/// <see cref="NvencCapabilities.IsAv1Available"/> probe is the gate;
/// callers must check before constructing this class.
/// </summary>
public sealed unsafe class NvencAv1Encoder : IVideoEncoder, IAsyncEncodedOutputSource
{
    ...
    public NvencAv1Encoder(int width, int height, int fps, long bitrate, int gopFrames, ID3D11Device device)
        : this(width, height, fps, bitrate, gopFrames, device, NvencEncodeOptions.Default)
    {
    }

    public NvencAv1Encoder(int width, int height, int fps, long bitrate, int gopFrames, ID3D11Device device, NvencEncodeOptions options)
    {
        _width = width;
        _height = height;
        _fps = Math.Max(1, fps);
        ...
        // its own resolved delegates against the populated table. AV1 is
        // gated on IsAv1Available — RTX 40+ / Arc / RDNA 3+ — separately
        // from the universal IsAvailable (which is "any NVENC at all").
        ...
        {
            throw new NvencException(NVENCSTATUS.NV_ENC_ERR_UNSUPPORTED_DEVICE,
                "ctor", "GPU does not advertise AV1 encode support");
        }
        _capabilities = caps;
        ...
        DebugLog.Write($"[nvenc] AV1 encoder initialized {width}x{height}@{_fps} {_bitrate} bps "
            + $"(P{_options.Preset} LOW_LATENCY, async, pool={_poolSize})");
    }
    ...
    private void InitializeEncoderConfig()
    {
        ...
        var status = _getPresetConfig(_encoder,
            NvencGuids.CodecAv1, presetGuid,
        ...
        // Override what we care about for Phase 1 baseline.
        preset.presetCfg.profileGUID = NvencGuids.Av1ProfileMain;
        ...
        init.encodeGUID = NvencGuids.CodecAv1;
    ...
    private void ApplyQualityOptions(ref NV_ENC_CONFIG cfg)
    {
        ...
        unsafe
        {
            // Overlay the AV1-specific config onto the codec union and
            // patch the bits we want.
            fixed (NV_ENC_CODEC_CONFIG* codecCfgPtr = &cfg.encodeCodecConfig)
            {
                var av1 = (NV_ENC_CONFIG_AV1_OVERLAY*)codecCfgPtr;
                // RepeatSeqHdr: emit a Sequence OBU at every Key Frame so a
                // receiver that joins mid-stream (or recovers from loss)
                // doesn't have to wait for the next implicit sequence
                // header. Same intent as H.264's RepeatSPSPPS.
                av1->bitfields |= Av1ConfigBits.RepeatSeqHdr;

                if (wantIntraRefresh)
                {
                    av1->bitfields |= Av1ConfigBits.EnableIntraRefresh;
                    var period = _options.IntraRefreshPeriodFrames > 0
                        ? (uint)_options.IntraRefreshPeriodFrames
                        : (uint)_gopFrames;
                    av1->intraRefreshPeriod = period;
                    av1->intraRefreshCnt = (uint)Math.Max(2, (int)(period / 30));
                    av1->idrPeriod = NvencApi.NVENC_INFINITE_GOPLENGTH;
                    cfg.gopLength = NvencApi.NVENC_INFINITE_GOPLENGTH;
                    DebugLog.Write($"[nvenc/av1] options: intraRefresh enabled period={period} cnt={av1->intraRefreshCnt}");
                }
                else
                {
                    cfg.gopLength = (uint)_gopFrames;
                    av1->idrPeriod = (uint)_gopFrames;
                }
            }
        }
    }
    ...
    public void UpdateBitrate(int bitsPerSecond)
    {
        ...
        preset.presetCfg.rcParams.version = NvencApi.NV_ENC_RC_PARAMS_VER;
        var presetGuid = NvencGuids.PresetByLevel(_options.Preset);
        var pcs = _getPresetConfig(_encoder, NvencGuids.CodecAv1, presetGuid,
            NV_ENC_TUNING_INFO.LowLatency, &preset);
        ...
        rec.reInitEncodeParams.encodeGUID = NvencGuids.CodecAv1;
    ...
    public void Dispose()
    {
        ...
        _freeSlots?.Dispose();
        DebugLog.Write("[nvenc] AV1 encoder disposed");
    }
